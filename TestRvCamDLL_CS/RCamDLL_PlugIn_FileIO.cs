...
namespace RvCamDLL_PlugIn_FileIO
{
...
#pragma warning disable CS8601 // 取消 warnning 
    ...
    public static class ClsRvCamDLL_PlugInDLL_FileIO
    {
        ...
        public static bool Process_Method_Save_File(IntPtr procAddr, ref TVectSimpleShape[] shapeData, ref string saveFileName)
        {
            ...
            using (SaveFileDialog SaveFileDialog = new SaveFileDialog())
            {
                ...
                if (SaveFileDialog.ShowDialog() == DialogResult.OK)
                {
                    ...
                    IntPtr pSetShpAry0 = IntPtr.Zero;
                    int shpAryLength = 0;

                    if (TReturnCode.rcSuccess == getMethodName(pSavedFn,  pSetShpAry0, shpAryLength))
                    {
        ...
        public static bool LoadLibraryMethod_And_Run(int dllFnID, int methodId, ref string loadSaveFileName)
        {
            bool ret = false;
            loadSaveFileName = "";
                ...
                if (hModule != IntPtr.Zero)
                {
                    ...
                    if (farProc != IntPtr.Zero)
                    {
                        ...
                        switch (methodId)
                        {
                            ...
                            case 1: //"Load_File"
                                if (Process_Method_Load_File(farProc, ref FVectShapeData, ref loadSaveFileName))
                                {
#if DEBUG
                                    //string sShps = "";
                                    //for (int i = 0; i < FVectShapeData.Length; i++)
                                    //{
                                    //    string shpName = string.Format("{0}({1})", FVectShapeData[i].ToString(), i);
                                    //    sShps += string.Format("\t{0}\n", shpName);

                                    //}
                                    //MessageBox.Show(string.Format("ShpAry Length : {0}\n\n{1} ", FVectShapeData.Length, sShps));
#endif
                                    ret = (FVectShapeData.Length>0);
                                }
                                break;

                            case 2:// "Process_TVectSimpleShapes"
                                ret = Process_Method_Process_TVectSimpleShapes(farProc, ref FVectShapeData, ref loadSaveFileName);
                                break;

                            case 3:// "Save_File"
                                ret = Process_Method_Save_File(farProc, ref FVectShapeData, ref loadSaveFileName);
                                break;
